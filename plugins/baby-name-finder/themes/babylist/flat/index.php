...
foreach (range('A', 'Z') as $char){
	$html.='<span class="baby_nf_each">';
	$html.='<a class="baby_nf_each_a" href="'.$baby_nf_archive_page_link.$permalink_joint.'satrts_with='.$char.'">'.strtoupper($char);
	$html.='</a>';
	$html.='</span>';
}
$html .= '</div>';

if(empty($_GET['satrts_with'])){
	$html .= '<div class="search-result job-list">';
	$html .= '<div class="baby_nf-list">';
	$html .= '<div class="single">Please click on desire Character';
	$html .= '</div>';
	$html .= '</div>';
	$html .= '<div class="clear"></div>';
	$html .= '</div>';
}else{
	$satrts_with = sanitize_text_field($_GET['satrts_with']);	
	$html .= '<div class="search-result job-list">';	
