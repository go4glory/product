...
if(empty($_GET['baby_nf_nationality'])){
	$baby_nf_nationality = '';
}else{
...
if(empty($_GET['baby_nf_religion'])){ 
	$baby_nf_religion = '';
}else{
...
$html .= '<div class="character-codes">';
foreach (range('A', 'Z') as $char){
	$html .= '<dl><dt>'.$char.'</dt></dl>';
}
$html .= '</div>';
...
if ( $wp_query->have_posts() ) :
	while ( $wp_query->have_posts() ) : $wp_query->the_post();	
		...
		if ( !empty( $baby_nf_meta_religion ) ) {
				//print_r($baby_nf_meta_religion);die;
			$html .= '
			<div class="baby_nf-meta '.$meta_baby_nf_meta_religion.'">
			<i class="fa fa-bars"></i>
			<a href="'.$baby_nf_archive_page_link.$permalink_joint.'baby_nf_religion='.$baby_nf_meta_religion.'">'.
			$baby_nf_religious_list[$baby_nf_meta_religion].'</a></div>';
		}
