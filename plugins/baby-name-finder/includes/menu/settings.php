...
/*
* @Author 		ParaTheme
* @Folder	 	baby-name-finder/includes/menu
...
class class_baby_nf_settings_page  {
	...
	public function baby_nf_settings_options($options = array()){


			$class_baby_nf_functions = new class_baby_nf_functions();
			
			$baby_nf_list_user_role = $class_baby_nf_functions->baby_nf_list_user_role();




			$options['Options'] = array(
								'baby_nf_list_per_page'=>array(
			...
			$options = apply_filters( 'baby_nf_settings_options', $options );
	...
	public function baby_nf_settings_options_form(){
		
			global $post;
			
			$baby_nf_settings_options = $this->baby_nf_settings_options();
			//var_dump($baby_settings_options);
			$html = '';
			...
			$i=1;
			foreach($baby_nf_settings_options as $key=>$options){
			if($i==1){
			...
			foreach($options as $option_key=>$option_info){
				...
				elseif($option_info['input_type'] == 'text-multi'){
					...
					foreach($input_args as $input_args_key=>$input_args_values){
						...
						$html_box.= '<label>'.$input_args_key.'<br/><input class="baby-bm-color" type="text" placeholder="" name="'.$option_key.'['.$input_args_key.']" value="'.$option_value[$input_args_key].'" /></label><br/>';	
						}
...
new class_baby_nf_settings_page();
...
if(empty($_POST['baby_nf_hidden']))
	{


		$class_baby_nf_settings_page = new class_baby_nf_settings_page();
		
			$baby_nf_settings_options = $class_baby_nf_settings_page->baby_nf_settings_options();
			
			foreach($baby_nf_settings_options as $options_tab=>$options){
	...
	{	
		if($_POST['baby_nf_hidden'] == 'Y') {
			...
			?>
			<div class="updated"><p><strong><?php _e('Changes Saved.', 'baby_nf' ); ?></strong></p></div>
...
<div class="wrap">
	...
	<div id="icon-tools" class="icon32"><br></div><?php echo "<h2>".__(baby_nf_plugin_name.' Settings', 'baby_nf')."</h2>";?>
		<form  method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
	<input type="hidden" name="baby_nf_hidden" value="Y">
        <?php settings_fields( 'baby_nf_plugin_options' );
				do_settings_sections( 'baby_nf_plugin_options' );
			
			
	$class_baby_nf_settings_page = new class_baby_nf_settings_page();
        echo $class_baby_nf_settings_page->baby_nf_settings_options_form(); 
...
<p class="submit">
                    <input class="button button-primary" type="submit" name="Submit" value="<?php _e('Save Changes','baby_nf' ); ?>" />
                </p>
		</form>
